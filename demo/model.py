...
class PlanarStandingHumanOnMovingPlatform(object):
    ...
    def _numerical_parameters(self):
        ...
        # These come from the yeadon software, see the park2004 example in
        # opty.
        p = {'leg_length': 0.878,
             'torso_inertia': 2.4811635063274338,
             'g': 9.81,
             'torso_com_length': 0.31449182352868588,
             'leg_com_length': 0.57159831527368432,
             'leg_inertia': 1.7991738234932386,
             'torso_mass': 48.830566595091696,
             'leg_mass': 32.125955184002557}
    ...
    def closed_loop_ode_func(self, time, reference_noise,
        ...
        rhs = generate_ode_function(self.forcing_vector_full,
                                    ...
                                    specifieds=self.specified.values()[-3:],
                                    generator='cython',
                                    constants_arg_type='array',
                                    specifieds_arg_type='function')

        p = np.array(self.open_loop_par_map.values())

        return rhs, controller, p
